...
from openedx.core.djangoapps.user_api.accounts.permissions import IsSuperUserOrCanDeactivateUser
from student.tests.factories import ContentTypeFactory, PermissionFactory, SuperuserFactory, UserFactory


class IsSuperUserOrCanDeactivateUserTest(TestCase):
    """ Tests for user deactivation API permissions """

    def setUp(self):
        super(IsSuperUserOrCanDeactivateUserTest, self).setUp()
        self.request = RequestFactory().get('/test/url')

    def test_api_permission_superuser(self):
        self.request.user = SuperuserFactory()

        result = IsSuperUserOrCanDeactivateUser().has_permission(self.request, None)
